********************************************************************************
****   Project: COVID-19 Reponses by States (Latest Update: 06/01/2020)
****   Producer: Wooserk Park
****   Steps: 1. Cleaning Data Using Master Data from Johns Hopkins Data
****         	 1.1. Worldwide Data Collecting (daily_reports)
****         	 1.2. Cleaning the data
****          2. Daily Data Producing
****   
****   Note: Data from: https://raw.githubusercontent.com/CSSEGISandData/COVID-19/master/csse_covid_19_data/csse_covid_19_daily_reports/  
****   Note2: Data will be updated every Sunday (Each weekly data contain Monday through Sunday observations).
********************************************************************************
...
********************************************************************************
** 1.1. US Data Collecting (daily_reports) ****************************
...
** Create week variable & Only kept Sunday due to duplicates **

drop last_update lastupdate
order date* admin2 province_state country_region lat long_ active combined_key fips /// ** this is the latest variable shape (post-Mar 21) 
...
**Matching provincestate if two same areas use differet names **

order date* province_state country_region confirmed deaths recovered
...
save "$dataoutput\rawdata\append_COVID_daily_cl.dta", replace
********************************************************************************

********************************************************************************
** 2. Daily Data Producing **
********************************************************************************	 
use "$dataoutput\rawdata\append_COVID_daily_cl.dta", clear	  

levelsof date1, local(levels)
foreach i of local levels {
preserve
keep if date1 == `i'
collapse (sum) confirmed deaths recovered (mean) date1, by (province_state)
order date1
rename (confirmed deaths recovered) (confirmed`i' deaths`i' recovered`i')
save "$dataoutput\daily_data\date_`i'.dta", replace
restore
}
foreach i of local levels {
use "$dataoutput\daily_data\date_21936.dta", clear
forvalues k = 21937/`i' {
merge 1:1 province_state using "$dataoutput\daily_data\date_`k'.dta", nogen
}
order province_state confirmed* deaths* recovered*
save "$dataoutput\daily_data\daily_merge.dta", replace
}

** Exporting **
use "$dataoutput\daily_data\daily_merge.dta", clear
reshape long confirmed deaths recovered, i(province_state) j(date)
...
order state_name state_code date cumulative_cases cumulative_deaths cumulative_recovered
export excel using "$dataoutput\daily_data\daily_summary.xls", sheet("Export from Stata") sheetmodify firstrow(variables)
